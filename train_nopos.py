...
if __name__ == "__main__":
    ...
    encoder = SDFEncoder()
    decoder = SDFDecoder()
    # encoder.load_state_dict(torch.load('checkpoints/epoch_encoder_%d.pth' % 0))z
    # decoder.load_state_dict(torch.load('checkpoints/epoch_decoder_%d.pth' % 0))
    
    encoder.load_state_dict(torch.load('checkpoints/no_pos/epoch_encoder_latest.pth'), strict=False)
    decoder.load_state_dict(torch.load('checkpoints/no_pos/epoch_decoder_latest.pth'), strict=False)
    ...
    for epoch in range(Epoch):
        ...
        for data in dataloader:
            ...
            print(one_sdf)
            logger.add_scalar("no_pos_call", one_sdf, train_idx)
            logger.add_scalar("no_pos_loss", loss.item(), train_idx)
            ...
            train_idx += 1
        torch.save(encoder.state_dict(), 'checkpoints/no_pos/epoch_encoder_%d.pth' % epoch)
        torch.save(decoder.state_dict(), 'checkpoints/no_pos/epoch_decoder_%d.pth' % epoch)
        torch.save(encoder.state_dict(), 'checkpoints/no_pos/epoch_encoder_latest.pth')
        torch.save(decoder.state_dict(), 'checkpoints/no_pos/epoch_decoder_latest.pth')
